...
def crop_img(img_numpy):
    ...
    half_width = width * 4 // 5
    half_height = height * 4 // 5
    
    # 确保裁剪区域不会超出图片边界
    max_width = min(center_x, width - center_x) * 2
    max_height = min(center_y, height - center_y) * 2
    half_width = min(half_width, max_width)
    half_height = min(half_height, max_height)
    
    x = center_x - half_width // 2
    y = center_y - half_height // 2
    w = half_width
    h = half_height
...
def is_file_open(filepath):
    try:
        with open(filepath, 'wb'):
            pass  # 尝试打开文件但不进行任何操作
        return False  # 如果没有异常，则文件未被打开
    except IOError:
        return True  # 如果捕获到IOError，则文件可能被打开
...
def calibration():
        ...
        fig, ax = plt.subplots()
        #sc = ax.scatter(x, y)
        img = Image.open("calibration.jpg")
        ax.imshow(img)
        ...
        def onclick(event):
            ...
            if event.dblclick:
                ...
                cali_points.append((event.xdata, event.ydata))
                ax.plot(event.xdata, event.ydata, 'ro')  # 绘制点击的点
                plt.draw()

            # 当有两个点时，计算并显示距离
            if len(cali_points) == 2:
                line_length = np.linalg.norm(np.array(cali_points[0]) - np.array(cali_points[1]))
                ax.text(0.5, 0.5, f'pixel_length: {line_length:.2f}', transform=ax.transAxes, fontsize=15, ha='center')
                #self.ui.label_cali.setText(str(int(self.line_length)))
                cali_points = []  # 重置点列表
                #main_flag = True
        ...
        # 设置显示参数
        #ax.set_title('Click to measure distance')

        # 展示图形
        plt.show(block=False) # plt.show() 会启动一个事件循环
        return line_length
