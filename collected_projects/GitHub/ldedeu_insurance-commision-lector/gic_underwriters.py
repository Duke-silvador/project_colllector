"""Statement GIC UNDERWRITERS: llega normalmente en Excel, pero se prepara para PDF/imagen
tambien (paginas_documento + extraccion geometrica, igual que TOWER HILL/ORCHID/SLIDE),
unificando ambos formatos en la misma fila canonica antes de resolver franquicia y guardar.
Tambien puede llegar en formato de cheque (como BASS/CRC GROUP), combinado o en paginas
separadas; como todavia no conocemos el layout real del talon de cheque, por ahora solo se
extrae la tabla de polizas de la pagina (el cheque no se valida por pagina como en CRC GROUP;
la conciliacion es mensual, igual que TOWER HILL).

El Excel trae su propia columna "Carrier" (la aseguradora real, ej. Mid-Continent Casualty
Company), pero eso es un dato de la aseguradora subyacente, no nuestro carrier interno: aqui
siempre se usa "GIC Underwriters" sin importar lo que diga esa columna.

Franquicia: se busca primero en Compass por numero de poliza; si no aparece ahi, se usa el
historico; si tampoco esta ahi, se prueba Company Search por nombre del cliente (busqueda
visual). Si nada de eso encuentra nada, queda sin franquicia y se marca en rojo.

Reparto: pass-through (igual que GRANADA/ORCHID/SLIDE) - la franquicia recibe exactamente el
mismo porcentaje que Del Toro, sin tabla de tarifas que mantener.

term_length se calcula directo de Eff Date/Exp Date del propio statement (no hace falta
Compass para esto, a diferencia de TOWER HILL que no trae fecha de vencimiento).
"""
import json
import re
...
CARRIER = 'GIC Underwriters'

HEADERS = ('Policy Number', 'Insured Name', 'Eff Date', 'Exp Date', 'Premium', 'Comm', 'Comm %')

FIELDS = ('policy_number', 'insured_name', 'effective_date', 'expiration_date', 'premium_amount',
          'commission_amount', 'del_toro_percent')
COLUMN_LABELS = (
...
def porcentaje_fraccion(valor):
    ...
    El statement real ya trae Comm % como fraccion (0.1), pero un numero con magnitud mayor
    a 1 se trata como porcentaje entero y se divide entre 100 por seguridad."""
...
def detectar_encabezado(sheet):
    ...
    for index, row in enumerate(sheet.iter_rows(values_only=True), 1):
        if required <= {_normalizar_encabezado(v) for v in row}:
            return index
    raise ValueError('No se encontró el encabezado completo de GIC UNDERWRITERS en la primera hoja.')
...
def preparar_statement(data, month, *, header=None):
    ...
    try:
        ...
        if not {_normalizar_encabezado(h) for h in HEADERS} <= positions.keys():
            raise ValueError('Faltan columnas de GIC UNDERWRITERS.')
        for number, cells in enumerate(sheet.iter_rows(min_row=header + 1), header + 1):
            ...
            try:
                premium = Decimal(str(source['Premium']))
                commission = Decimal(str(source['Comm']))
                if not all(v.is_finite() for v in (premium, commission)):
                    raise InvalidOperation
            except InvalidOperation:
                raise ValueError(f'Fila {number}: Premium o Comm inválido.') from None
            try:
                rate = porcentaje_fraccion(source['Comm %'])
            except ValueError:
                raise ValueError(f'Fila {number}: Comm % inválido.') from None
            try:
                effective = _fecha_celda(cells[positions[_normalizar_encabezado('Eff Date')]].value)
                expiration = _fecha_celda(cells[positions[_normalizar_encabezado('Exp Date')]].value)
            except ValueError as exc:
                raise ValueError(f'Fila {number}: fecha inválida ({exc}).') from None
            ...
            result.append(dict(
                ...
                policy_number=policy, insured_name=source['Insured Name'].strip(),
                ...
                term_length=calcular_term_length(effective, expiration),
            ))
        if not result:
...
def _normalizar_palabra(texto):
    # A diferencia de otros carriers, aqui se conserva '%' porque 'Comm' y 'Comm %' son la
    # unica forma de distinguir Commission Amount de Del Toro % en el encabezado: quitar el
    # simbolo dejaria ambos como la misma palabra ('comm').
...
def extraer_tabla_pagina(blocks, *, detalles=False):
    """Encabezados detectados por texto (no por posicion fija). Se exigen todas las columnas
    del statement (todas son obligatorias: no hay campos informativos opcionales aqui)."""
    ...
    for field in FIELDS:
        ...
        try:
            if field in ('effective_date', 'expiration_date'):
                fecha(value)
            elif field in ('premium_amount', 'commission_amount'):
                importe(value)
...
def construir_filas(rows, month):
    ...
    for number, row in enumerate(rows, 1):
        ...
        policy = '' if policy_raw.upper() in ('', 'N/A') else policy_raw
        effective = fecha(row['effective_date'])
        expiration = fecha(row['expiration_date'])
        result.append(dict(
            ...
            effective_date=effective, expiration_date=expiration,
            premium_amount=str(importe(row['premium_amount'])), commission_amount=str(importe(row['commission_amount'])),
...
# ---------------------------------------------------------------------------
# Franquicia: Compass por numero de poliza primero, historico como respaldo, y por
# ultimo Company Search por nombre del cliente.
# ---------------------------------------------------------------------------
...
def completar_franquicias(connection, rows):
    """GIC UNDERWRITERS no trae codigo de agencia ni oficina en el statement. La franquicia
    se busca primero en Compass por numero de poliza; si no aparece ahi (o Compass no
    resuelve una oficina traducible), se usa el historico. Si tampoco esta en el historico,
    se prueba por ultimo Company Search por nombre del cliente. Si nada de eso encuentra
    nada, queda sin franquicia y se marca en rojo. producer_name se toma del nombre real de
    la oficina resuelta (office_name)."""
    if not rows:
        return
    cursor = connection.cursor(dictionary=True)
    try:
    ...
    def usar_historico(row, numero, mensaje_no_encontrado):
        franquicia, office_id, state = _franquicia_historica(connection, numero, offices)
        if franquicia:
            ...
            row.update(franchise_number=franquicia, franchise_number_source='historico',
                       office_id=office_id, state=state,
    ...
    for row in rows:
        row.update(franchise_number=None, franchise_number_source=None, office_id=None, state=None,
                   compass_policy_id=None, policy_status=None, producer_name=None)
...
def guardar(rows, month, file_name, contenido, source_page, original_ocr):
    ...
    locked = False
    try:
        cursor.execute("SELECT GET_LOCK('staging_hub.st_gic_underwriters_raw.import',10) AS acquired")
        locked = cursor.fetchone()['acquired'] == 1
        if not locked:
            raise ValueError('Hay otra importación de GIC UNDERWRITERS en curso.')
        ...
        payloads = {row['source_row']: json.dumps({'reviewed': row['source_data'], 'original_ocr': original_ocr},
                   ...
                   for row in rows}
        cursor.execute('SELECT id, source_row, source_data FROM staging_hub.st_gic_underwriters_raw'
        ...
        for row in rows:
            ...
            previo = existentes.get(row['source_row'])
            valores = (
                row['policy_number'] or 'N/A', row['insured_name'], row['effective_date'], row['expiration_date'],
                row['premium_amount'], row['commission_amount'], row['del_toro_percent'], row.get('term_length'),
                row.get('compass_policy_id'), row.get('policy_status'), row.get('producer_name'),
                row.get('franchise_number'), row.get('franchise_number_source'), row.get('office_id'),
                row.get('state'), payloads[row['source_row']],
            )
            if previo is not None:
                cursor.execute(
                    'UPDATE staging_hub.st_gic_underwriters_raw SET policy_number=%s, insured_name=%s,'
                    ' effective_date=%s, expiration_date=%s, premium_amount=%s, commission_amount=%s,'
                    ' del_toro_percent=%s, term_length=%s, compass_policy_id=%s, policy_status=%s, producer_name=%s,'
                    ' franchise_number=%s, franchise_number_source=%s, office_id=%s, state=%s,'
                    ' source_data=%s WHERE id=%s', valores + (previo['id'],))
                updated += 1
            else:
                cursor.execute(
                    'INSERT INTO staging_hub.st_gic_underwriters_raw (policy_number, insured_name,'
                    ' effective_date, expiration_date, premium_amount, commission_amount, del_toro_percent,'
                    ' term_length, compass_policy_id, policy_status, producer_name,'
                    ' franchise_number, franchise_number_source,'
        ...
        try:
            if locked:
                cursor.execute("SELECT RELEASE_LOCK('staging_hub.st_gic_underwriters_raw.import')")
                cursor.fetchone()
        finally:
            cursor.close()
            connection.close()
...
def cargar_comisiones_excel(connection, snapshot, carrier=None, state=None, business_line=None, *, consultar_tipos=False):
    ...
    en un solo reparto (varios statements de GIC UNDERWRITERS para un mismo mes se concilian
    juntos). Pass-through: la franquicia recibe exactamente el mismo % que Del Toro."""
    cursor = connection.cursor(dictionary=True)
    try:
        base = 'SELECT r.* FROM staging_hub.st_gic_underwriters_raw r WHERE r.accounting_month=%s'
        ...
        if not rows:
            raise ValueError('No hay registros guardados de GIC UNDERWRITERS para este mes.')
        calculated = []
        for row in rows:
            row['carrier'] = CARRIER
            ...
            row['del_toro_commission'] = row['commission_amount']
            row['franchise_percent'] = row['del_toro_percent']
            row['franchise_commission'] = row['commission_amount']
            row['commission_alert'] = None if row.get('franchise_number') else (
                f"Póliza {row.get('policy_number') or 'sin número'}: sin franquicia en Compass, histórico ni búsqueda visual")
