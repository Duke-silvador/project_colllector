"""Carga TOWER HILL. Se espera Excel, pero se prepara para que llegue en PDF o imagen tambien
(igual que ORCHID/SLIDE: se lee con documentos.paginas_documento y se extrae la tabla por
geometria). Ambos formatos producen la misma fila antes de resolver franquicia y guardar.
...
from tower_hill import (FIELDS, detectar_encabezado, preparar_statement, extraer_tabla_pagina,
...
ETIQUETAS_CAMPO = {
    'policy_number': ('Póliza', 'Policy Number'), 'effective_date': ('Fecha efectiva', 'Effective date'),
    'insured_name': ('Asegurado', 'Named Insured'), 'transaction_type': ('Term', 'Term'),
    'written_premium': ('Written Premium', 'Written Premium'), 'premium_amount': ('Commission Premium', 'Commission Premium'),
    'del_toro_percent': ('Rate', 'Rate'), 'commission_amount': ('Commission Amount', 'Commission Amount'),
    'form': ('Form', 'Form'), 'producing_agent': ('Producing Agent', 'Producing Agent'),
    'submitted_agent': ('Submitted Agent', 'Submitted Agent'),
}
...
def _tabla_editable(extracted, suffix):
    state_key = 'tower_hill_rows_' + suffix
    version_key = 'tower_hill_rows_version_' + suffix
    ...
    editor_key = f'tower_hill_rows_editor_{suffix}_{st.session_state[version_key]}'
    ...
    def save_edits():
        ...
        st.session_state['tower_hill_confirm_' + suffix] = False
...
def _bloque_excel(archivo, month):
    ...
    mostrar_tabla([{k: v for k, v in r.items() if k != 'source_data'} for r in rows], 'tower_hill_preview_' + key, st=st)
    ...
    _revisar_duplicados('st_tower_hill_raw', rows, month, file_id, 'policy_number', 'commission_amount')
    confirmed = st.checkbox(texto('Revisé los registros y confirmo la importación.',
                                  'I reviewed the records and confirm the import.'), key='tower_hill_confirm_' + key)
    if st.button(texto('Importar faltantes y continuar', 'Import missing and continue'), disabled=not confirmed,
                 key='tower_hill_save_' + key, type='primary'):
        try:
            with st.spinner(texto('Guardando TOWER HILL…', 'Saving TOWER HILL…')):
...
def _bloque_pdf(archivo, month):
    ...
    draft_key = 'tower_hill_draft_' + identity
    if st.button(texto('Extraer campos para revisión', 'Extract fields for review'), key='tower_hill_extract_' + identity):
        try:
...
def mostrar(month):
    ...
    st.caption(texto(
        ...
        'suelta. Puedes subir uno o varios documentos a la vez; se importan por separado pero se concilian y '
        'exportan juntos, más abajo.',
        'The statement is normally Excel, but it can also arrive as a PDF with real text, a scanned PDF, or a '
        'standalone image. You can upload one or more documents at once; they are imported separately but '
        'reconciled and exported together, further down.'))
    st.caption(texto(
        'El statement no trae código de agencia ni oficina. La franquicia se busca en el histórico por número de '
        'póliza; si no está ahí, la fila queda sin franquicia y se marca para revisión manual.',
        "The statement does not include an agency code or office. The franchise is looked up in history by policy "
        'number; if not found there, the row is left without a franchise and flagged for manual review.'))
    archivos = st.file_uploader(texto('Statements TOWER HILL (Excel, PDF o imagen)', 'TOWER HILL statements (Excel, PDF or image)'),
                                type=['xlsx', 'xlsm', 'pdf', 'png', 'jpg', 'jpeg'], accept_multiple_files=True,
                                key='tower_hill_files')
    if not archivos:
        return
    ...
    st.caption(texto(
        'Combina todos los statements de TOWER HILL ya guardados para este mes contable, sin importar de qué '
        'archivo vinieron.', "Combines every TOWER HILL statement already saved for this accounting month, "
        'regardless of which file it came from.'))
    accounting_month = datetime.strptime(month, '%Y-%m').date()
    connection = conectar()
    try:
        cursor = connection.cursor()
        cursor.execute('SELECT SUM(commission_amount) FROM staging_hub.st_tower_hill_raw WHERE accounting_month=%s',
                       (accounting_month,))
    ...
    mostrar_conciliacion(snapshot, 'tower_hill_month_' + month, 'TOWER HILL')
